...
(function(pa) {
    ...
    document.addEventListener('keydown', function(event) {
      ...
      function opModeCommand(args, sendInfo) {
          var usageMessage = "Kullanım: /opmode <sayı>. Varsayılan değer 2'dir.";
          
          if (args.length == 2) {
              var value = parseInt(args[1]);
              if (!isNaN(value)) {
                  sldVar = value;
                  sendInfo(`OP mode ${value} olarak ayarlandı.`);
              } else {
                  sendInfo(usageMessage);
              }
          } else {
      ...
      function chatBubbleCommand(args, sendInfo) {
          var usageMessage = "Kullanım: /kutu <on|off>";
          ...
          if (args.length > 0) {
              ...
              if (value == "off") {
                  ...
                  sendInfo("Chat kutucuğu kapatıldı");
              }
              else if (value == "on") {
                  window.localStorage.setItem("chatBubble", "on");
                  sendInfo("Chat kutucuğu açıldı");
              }
              else {
                  sendInfo(usageMessage);
              }
      ...
      function chatBubbleInfo(sendInfo) {
          if (getChatBubbleValue()) {
              sendInfo("Chat kutucuğu açıldı - bir takıma girdiğinde chati aç.");
          }
      }
    ...
    bb.O = "<div class='chatbox-view'><div class='chatbox-view-contents'><div data-hook='drag' class='drag'></div><div data-hook='log' class='log subtle-thin-scrollbar'><div data-hook='log-contents' class='log-contents'><p>OP MODE</p></div></div><div class='autocompletebox' data-hook='autocompletebox'></div><div class='input'><input data-hook='input' type='text' /></div></div></div>";
